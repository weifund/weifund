...
contract CrowdFundrr 
{
    ...
    function newCampaign(address beneficiary, uint goal, uint timelimit, uint category)
    {
    ...
    function getShares(uint cid, address addr) returns (uint shares)
    {
        Campaign c = campaigns[cid];
        if(c.numFunders != 0 && c.toFunder[addr] >= 0){
            Funder f = c.funders[c.toFunder[addr]];
            return f.shares;
        }
    }
    
    function isFunder(uint cid, address addr) returns (bool b)
    {
        Campaign c = campaigns[cid];
    ...
    function transferShares(uint cid, address addr, uint amount)
    {
        Campaign c = campaigns[cid];
        if(c.numFunders != 0 && amount > 0 && c.toFunder[msg.sender] >= 0){
            Funder f = c.funders[c.toFunder[msg.sender]];
            
            if(f.shares > 0 && f.shares < amount){
                f.shares -= amount;
                if(isFunder(cid, addr)){
                    Funder t_f = c.funders[c.toFunder[addr]];
                    t_f.shares += amount;
                }else{
                    uint new_id = c.numFunders += 1;
                    Funder new_f = c.funders[new_id];
                    new_f.shares = amount;
                    c.toFunder[addr] = new_id;
                }
            }
        }
    }
