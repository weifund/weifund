	...
	'userContributions': function(){
		try {
			...
			if(web3.isAddress(contributionIDorAddress)) {
				return Contributions.find({campaignID: campaignID, contributor: contributionIDorAddress});
			}else{
				return Contributions.find({campaignID: campaignID, id: contributionIDorAddress});
			}
		}catch(err){
			return TemplateVar.set('state', {isError: true, error: err});
		}
	},
});
    ...
    'click #contributorLookup': function(event, template){   
		...
		var lookupCallback = function(err, contribution){
			...
			if(!contribution.isValid)
				return TemplateVar.set(template, 'state', {isError: true, error: 'Contribution invalid'});

			Contributions.upsert({campaignID: contributionCampaignID, id: contribution.id}, contribution);
			TemplateVar.set(template, 'state', {isContributor: true});
		};
