    ...
    **/
    ...
    'click #personaRegister': function(event, template){
		...
		ipfs.addJson(personaObject, function(err, ipfsHash){
			...
			TemplateVar.set(template, 'registerState', {isMining: true, ipfsHash: ipfsHash, ipfsHashHex: ipfsHashHex});

			objects.contracts.PersonaRegistry.setPersonaAttributes(ipfsHashHex, transactionObject, function(err, result){
				if(err)
					return TemplateVar.set(template, 'registerState', {isError: true, error: err});

				TemplateVar.set(template, 'registerState', {isMining: true, transactionHash: result, ipfsHash: ipfsHash, ipfsHashHex: ipfsHashHex});
			});
		});
    },
    ...
    **/
    ...
    'click #personaLookup': function(event, template){
		...
		objects.helpers.importPersona(addr, function(err, persona){
			...
			return TemplateVar.set(template, 'lookupState', {isSuccess: true, persona: persona});
		});
    },
});